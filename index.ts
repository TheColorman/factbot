...
const saveImagae = (attachment: MessageAttachment | MessageEmbed) => {
    ...
    const images = JSON.parse(fs.readFileSync("./data/download.json", "utf8"))
        .images as string[];
    // Check if url exists
...
client.on("messageCreate", async (message) => {
    ...
    if (mentionsSelf && message.content.toLowerCase().includes("meme")) {
        sendMessage(async () => {
            const images = JSON.parse(
                fs.readFileSync("./data/download.json", "utf8")
            ).images as string[];
    ...
    if (previousMessageUser.equals(message.client.user)) {
        ...
        if (
            insults.some((insult) =>
                message.content.toLowerCase().includes(insult)
            )
        ) {
            sendMessage(async () =>
                safeReply(
                    ...
                    await createOutput(
                        insultReplies[
                            Math.floor(Math.random() * insultReplies.length)
                        ],
                        message
                    )
                )
    ...
    if (previousMessageUser.equals(message.client.user)) {
        ...
        if (
            praise.some((praise) =>
                message.content.toLowerCase().includes(praise)
            )
        ) {
            sendMessage(async () =>
                safeReply(
                    ...
                    await createOutput(
                        praiseReplies[
                            Math.floor(Math.random() * praiseReplies.length)
                        ],
                        message
                    )
                )
    ...
    if (message.content.includes("cunny") && Math.random() > 0.5) {
        ...
        const file = new MessageAttachment(
            path.resolve(path.join(__dirname, "images", image))
        );
    ...
    if (message.content.toLowerCase().includes("bug") && Math.random() < 0.2) {
        ...
        const file = new MessageAttachment(
            path.resolve(path.join(__dirname, "images", "bug", bug))
        );
        sendMessage(() => safeReply(message, { files: [file] }));
        return;
    }
    ...
    if (
        greetings.some(
            (w) => message.content.startsWith(w) && Math.random() < 0.1
        )
    ) {
        sendMessage(async () =>
            safeReply(
